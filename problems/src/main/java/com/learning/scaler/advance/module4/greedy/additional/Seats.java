...
public class Seats {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(seats.seats("....x..xx...x.."));
        System.out.println(seats.seats("....xxxx"));
    }

    public int seats(String A) {
        List<Integer> filledSeats = new ArrayList<>();
        char[] chars = A.toCharArray();
        for (int index = 0; index < A.toCharArray().length; index++) {
            if (chars[index] == 'x') {
                filledSeats.add(index);
            }
        }
        // if it is already in ordered situation
        if (filledSeats.isEmpty() || (filledSeats.size() == (filledSeats.get(filledSeats.size() - 1) + 1))) return 0;
        int mid = filledSeats.size() / 2, left = mid - 1, right = mid + 1, k = 1, result = 0;
        while (left >= 0) {
            result += (filledSeats.get(mid) - k - filledSeats.get(left));
            left--;
            k++;
        }
        //reset k
        k = 1;
        while (right < filledSeats.size()) {
            result += (filledSeats.get(right) - k - filledSeats.get(mid));
            right++;
            k++;
        }
        return result;
